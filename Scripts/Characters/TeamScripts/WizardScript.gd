...
onready var tween=get_parent().get_parent().get_parent().get_node("Camera/UIHolder/HealthHolder/WizardHealthHolder/Tween")
onready var healthBar=get_parent().get_parent().get_parent().get_node("Camera/UIHolder/HealthHolder/WizardHealthHolder/Health")
onready var healthBarUnder=get_parent().get_parent().get_parent().get_node("Camera/UIHolder/HealthHolder/WizardHealthHolder/HealthUnder")
...
func damage(damageNo):
	if dead!=true:
		...
		tween.start()
		MusicHandler.get_node("WizardHitSound").play()
		...
		if health<=0:
			...
			get_parent().get_parent().get_node("EnemyHolder/Enemy").characterCount.remove(3)
...
func _physics_process(_delta):
	...
	if dead==true:
		...
		stunned=false
		iced=false
		GlobalVariableHolderScript.WizardDead=true
