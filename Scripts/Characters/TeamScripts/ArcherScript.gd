...
onready var tween=get_parent().get_parent().get_parent().get_node("Camera/UIHolder/HealthHolder/ArcherHealthHolder/Tween")
onready var healthBar=get_parent().get_parent().get_parent().get_node("Camera/UIHolder/HealthHolder/ArcherHealthHolder/Health")
onready var healthBarUnder=get_parent().get_parent().get_parent().get_node("Camera/UIHolder/HealthHolder/ArcherHealthHolder/HealthUnder")
...
func damage(damageNo):
	if dead!=true:
		...
		tween.start()
		MusicHandler.get_node("ArcherHitSound").play()
		...
		if health<=0:
			...
			get_parent().get_parent().get_node("EnemyHolder/Enemy").characterCount.remove(2)
...
func _physics_process(_delta):
	...
	if dead==true:
		...
		stunned=false
		iced=false
		GlobalVariableHolderScript.ArcherDead=true
	...
	if posioned==true:
		...
		stunned=false
		iced=false
		$PoisonCPUParticles.emitting=true
	if posioned==false:
		$PoisonCPUParticles.emitting=false
