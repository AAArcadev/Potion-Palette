...
onready var tween=get_parent().get_parent().get_parent().get_node("Camera/UIHolder/HealthHolder/KnightHealthHolder/Tween")
onready var healthBar=get_parent().get_parent().get_parent().get_node("Camera/UIHolder/HealthHolder/KnightHealthHolder/Health")
onready var healthBarUnder=get_parent().get_parent().get_parent().get_node("Camera/UIHolder/HealthHolder/KnightHealthHolder/HealthUnder")

var health=180
export var maxHealth=180
export var attack=30
export var defence=15
...
func damage(damageNo):
	if dead!=true:
		...
		MusicHandler.get_node("KnightHitSound").play()
		...
		if health<=0:
			...
			get_parent().get_parent().get_node("EnemyHolder/Enemy").characterCount.remove(0)
...
func _physics_process(_delta):
	...
	if dead==true:
		...
		stunned=false
		iced=false
		GlobalVariableHolderScript.knightDead=true
	if health>maxHealth:
		health=maxHealth
